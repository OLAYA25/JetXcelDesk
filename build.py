...
hbb_name = 'jetxceldesk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: jetxceldesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: jetxceldesk <info@jetxceldesk.com>
Homepage: https://jetxceldesk.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/jetxceldesk')
    system2('mkdir -p tmpdeb/etc/jetxceldesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/jetxceldesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/jetxceldesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/jetxceldesk/')
    system2(
        'cp ../res/jetxceldesk.service tmpdeb/usr/share/jetxceldesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/jetxceldesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/jetxceldesk.svg')
    system2(
        'cp ../res/jetxceldesk.desktop tmpdeb/usr/share/applications/jetxceldesk.desktop')
    system2(
        'cp ../res/jetxceldesk-link.desktop tmpdeb/usr/share/applications/jetxceldesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/jetxceldesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/jetxceldesk/')
    system2(
        'cp ../res/pam.d/jetxceldesk.debian tmpdeb/etc/pam.d/jetxceldesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/jetxceldesk/files/polkit && chmod a+x tmpdeb/usr/share/jetxceldesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb jetxceldesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('jetxceldesk.deb', '../jetxceldesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/jetxceldesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/jetxceldesk/')
    system2(
        'cp ../res/jetxceldesk.service tmpdeb/usr/share/jetxceldesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/jetxceldesk.png')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibjetxceldesk.dylib target/release/liblibjetxceldesk.dylib")
    os.chdir('flutter')
    system2('flutter build macos --release')
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/JetXcelDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"JetXcelDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon JetXcelDesk.app 200 190 --hide-extension JetXcelDesk.app jetxceldesk.dmg ./build/macos/Build/Products/Release/JetXcelDesk.app")
    os.rename("jetxceldesk.dmg", f"../jetxceldesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libjetxceldesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libjetxceldesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/jetxceldesk.exe')
    os.chdir('../..')
    if os.path.exists('./jetxceldesk_portable.exe'):
        os.replace('./target/release/jetxceldesk-portable-packer.exe',
                   './jetxceldesk_portable.exe')
    else:
        os.rename('./target/release/jetxceldesk-portable-packer.exe',
                  './jetxceldesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/jetxceldesk_portable.exe')
    os.rename('./jetxceldesk_portable.exe', f'./jetxceldesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/jetxceldesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/jetxceldesk.exe target/release/JetXcelDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\jetxceldesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/JetXcelDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/jetxceldesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/jetxceldesk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/jetxceldesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv jetxceldesk-%s-0-x86_64.pkg.tar.zst jetxceldesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/jetxceldesk-%s-0.x86_64.rpm ./jetxceldesk-%s-fedora28-centos8.rpm' % (
                version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/jetxceldesk-%s-0.x86_64.rpm ./jetxceldesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall rustdesk.rpm
    else:
        if flutter:
    ...
    '''.format(pa, version))
                ...
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                    'cp res/jetxceldesk.service tmpdeb/usr/share/rustdesk/files/systemd/')
                system2(
                ...
                os.system('cp pam.d/jetxceldesk.debian tmpdeb/etc/pam.d/rustdesk')
