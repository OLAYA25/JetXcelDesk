...
1. Use a temporary file `/tmp/jetxceldesk_<uuid>` as a placeholder in the pasteboard.
2. Create temp file in `/tmp/.jetxceldesk_*`.
...
Use `fsevent` to observe the paste operation with the source file `/tmp/.jetxceldesk_*`.
