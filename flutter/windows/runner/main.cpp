...
typedef char** (*FUNC_JETXCELDESK_CORE_MAIN)(int*);
typedef void (*FUNC_JETXCELDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_JETXCELDESK_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libjetxceldesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libjetxceldesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_JETXCELDESK_CORE_MAIN jetxceldesk_core_main =
      (FUNC_JETXCELDESK_CORE_MAIN)GetProcAddress(hInstance, "jetxceldesk_core_main_args");
  if (!jetxceldesk_core_main)
  {
    std::cout << "Failed to get jetxceldesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_JETXCELDESK_FREE_ARGS free_c_args =
  ...
  int args_len = 0;
  char** c_args = jetxceldesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "JetXcelDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"JetXcelDesk";
  FUNC_JETXCELDESK_GET_APP_NAME get_jetxceldesk_app_name = (FUNC_JETXCELDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_jetxceldesk_app_name");
  if (get_jetxceldesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_jetxceldesk_app_name(app_name_buffer, 512) == 0) {
