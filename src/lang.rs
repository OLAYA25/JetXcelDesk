...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_jetxceldesk() {
            if s.contains("JetXcelDesk")
                && !name.starts_with("upgrade_jetxceldesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("JetXcelDesk") {
                    s = s.replace("JetXcelDesk", &app_name);
                } else {
                    // https://github.com/OLAYA25/JetXcelDesk-server-pro/issues/845
                    // If app_name contains "JetXcelDesk" (e.g., "JetXcelDesk-Admin"), we need to avoid
                    // replacing "RustDesk" within the already-substituted app_name, which would
                    // cause duplication like "JetXcelDesk-Admin" -> "JetXcelDesk-Admin-Admin".
                    //
