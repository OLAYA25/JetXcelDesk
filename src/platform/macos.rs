...
const UPDATE_TEMP_DIR: &str = "/tmp/.jetxceldeskupdate";
...
// remove just one app from all the permissions: tccutil reset All com.carriez.jetxceldesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.carriez.jetxceldesk", &bundleid);
    }
    s = s.replace("jetxceldesk", &crate::get_app_name().to_lowercase());
    s = s.replace("JetXcelDesk", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/JetXcelDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another JetXcelDesk.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // JetXcelDesk.app/Contents/MacOS/JetXcelDesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/JetXcelDeskUpdate";
